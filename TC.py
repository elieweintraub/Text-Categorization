...
import nltk, math, cPickle, re, os
...
class InvertedIndex:
	...
	def __init__(self, token_params = 'default'):
		...
		if token_params == 'default': 
			...
			self.CASE_INSENSITIVE = int(token_params[0])  # flag for whether or not tokenization should be case-insensitive
			self.STOP_LIST = int(token_params[1])         # flag for whether or not tokenization should include a stop list
			self.POS = int(token_params[2])               # flag for whether or not tokenization should include POS tagging
	...
	def categorizeTexts(self): 
		...
		output_filename = raw_input('Please specify the name for the output file containing the labeled test documents: ')
...
if __name__ == "__main__":
	...
	if train:
		tokenization_parameters = input('Would you like to use the default tokenization parameters (0) or select your own (1): ')
		if tokenization_parameters:
			...
			token_params = raw_input('Please enter your choice of tokenization parameters: ')
		else:
		...
		save= input('Would you like to save a representation of the trained system for future use (0=>no,1=>yes): ')	
		if save:
